...
def main():
    ...
    # 查找 JSON 快照文件
    snapshots = sorted(
        glob.glob(os.path.join(data_dir, "fanqie_nan_new_ranks_*.json"))
    )
    ...
    if args.date:
        ...
        target_path = os.path.join(
            data_dir, f"fanqie_nan_new_ranks_{target_date_compact}.json"
        )
    ...
    for s in snapshots:
        ...
        # fanqie_nan_new_ranks_YYYYMMDD.json -> YYYY-MM-DD
